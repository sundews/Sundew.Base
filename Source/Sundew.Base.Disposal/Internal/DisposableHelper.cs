...
namespace Sundew.Base.Disposal.Internal
{
    ...
    internal static class DisposableHelper
    {
        ...
        public static async System.Threading.Tasks.ValueTask DisposeAsync(IEnumerable<object>? disposables, IDisposableReporter? disposableReporter)
        {
            if (disposables != null)
            {
                if (disposableReporter != null)
...
#if NETSTANDARD2_1
        private static async System.Threading.Tasks.ValueTask DisposeItemPreferDispose(object item)
        {
            switch (item)
            {
                case IDisposable disposable:

...
public static class ResultImmutableListExtensions
{
    ...
    {
        return result.IsSuccess ? immutableList : (TList)immutableList.AddRange(result.Error.GetItems().WhereNotNull());
    }
    ...
    public static TList TryAddAnyErrors<TList, TItem>(this TList immutableList, R<Failed<TItem>> result)
        where TList : IImmutableList<TItem>
    {
        return result.HasError ? (TList)immutableList.AddRange(result.Error.GetItems().WhereNotNull()) : immutableList;
    }

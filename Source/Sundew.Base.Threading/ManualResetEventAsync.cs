...
public sealed class ManualResetEventAsync
{
    ...
    public async Task<bool> WaitAsync(Cancellation cancellation)
    {
        ...
        try
        {
            await this.manualResetEventAsync.WaitAsync(enabler.Token).ConfigureAwait(false);
            return true;
        }
        catch (OperationCanceledException)
        {
    ...
    public async Task<bool> WaitAsync(TimeSpan timeSpan)
    {
        ...
        try
        {
            await this.manualResetEventAsync.WaitAsync(cancellationTokenSource.Token).ConfigureAwait(false);
            return true;
        }
        catch (OperationCanceledException)
        {
